...
- `check:api` 补的正是上一段里**最硬的那半**：「产物是否还反映源码」此前**没有人真的在扛**。`check:gen` 只是把生成器重跑一遍逐字节比对，而两边跑的是**同一份解析器** —— 产物再错也自洽。`gen-docs.py` 开头那句「API 内容直接取自各组件实际的 props 定义，保证与代码同步」、`types/index.d.ts` 开头那句「由各组件实际的 props 定义自动生成，与源码保持同步」，都没有落点。这一轮撞上两处：① **生成器会静默丢 prop** —— `parse_props` 按顶层逗号切段，「prop 上面单独一行写 `// 说明`」会让那行注释跟着下一个 prop 进同一段，旧写法 `if seg.startswith('//'): continue` 把**整个 prop 一起丢掉**；`vui-tag` 的 `text` 就这么从 `types/index.d.ts` 与 `docs/API.md` 里消失了（TS 用户写 `<vui-tag text="x">` 没有类型提示、文档查不到），而 13 条检查一路全绿 —— 实测把生成器退回旧写法重跑，`check:gen` 依然 exit 0，只有 `check:api` 红并点名「产物缺 text」。修法是只剔「整行都是注释」的行、**且空行也要跳过**（第一版漏了空行，补丁看起来改了、`text` 依然不见），两个生成器同源同修。② **插槽契约整个缺失** —— 20 个组件模板里有 28 个 `<slot>`，JSDoc 里 `@slot` **一个都没有**；`gen-docs.py` 从第 1 天起就支持 `@slot` 并会渲染「插槽」小节，于是 `docs/API.md` 里 0 个插槽小节，而 README 明写「完整 API（属性 / 事件 / 插槽）见 docs/API.md」。现在 28 个插槽全部登记（含 `vui-table` 那个由 `column.key` 决定的动态槽 —— 静态抓不到，必须写进 `DYNAMIC_SLOTS` 登记表，否则它会**静默退出**整张对账表），API.md 第一次有了 20 个插槽小节（名称 / 作用域参数 / 说明）。`props-no-dead` 那一组用的是**必要条件的近似**：名字在整库剥注释的代码里一次都不出现 = 一定没人读（实测抓出 `vui-chat-input.stopText`：文档、类型、API.md 都承诺了它，模板里却只画一个方块图标，用户 `stop-text="暂停"` 设了没反应）；反向不成立，所以方向偏保守 —— `vui-form` 的 `model` / `rules` / `labelWidth` 是被 `vui-form-item` 经 `inject('vuiForm')` 读的，纯按单文件扫会误伤。**一个会误报的检查最后一定会被加白名单加到失效**，宁可漏报。该脚本还带 9 条构造输入的自证（`propsOf` 的注释/空行形状、`@slot` 与 `<slot>` 的三态、`:name` 不得被当成字面量 —— 这条自证正是首跑抓出来的实现 bug），守卫自己坏掉不会安静地全绿。
...
**假红也是缺陷，而且比漏检更贵。** `check:components` 曾在 Windows 上偶发假红：临时副本删不掉时 `fs.rmSync` 抛 `EBUSY`，而那一下正好在 `finally` 里，被外层 catch 收成「自检执行失败」→ 整条 `check:all` 判红，报的还是一件跟组件结构毫无关系的事（同一份代码连跑三次：一次红两次绿）。现在删副本走 `rmTemp()`：退避重试几次，仍删不掉就**只打印警告并留着**（系统临时目录里的残留副本不是包里的缺陷），并配两条锁防止有人再写回裸删。**检查一旦会偶发假红，人就会开始绕过它，它守的东西也就全没了 —— 修假红和修漏检同样重要。**
